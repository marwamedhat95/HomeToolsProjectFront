...
export default function VideosManagement() {
  ...
  const handleUpdate = async (e) => {
    ...
    try {
      await axios.put(`https://localhost:5000/api/videos/${editPopup._id}`, formData);
  ...
  const handleDelete = async () => {
    await axios.delete(`https://localhost:5000/api/videos/${deletePopup}`);
    setDeletePopup(null);
    fetchVideos();
  };
  ...
  const handleAddVideo = async (e) => {
    ...
    await axios.post("https://localhost:5000/api/videos", formData);
        ...
        {videos.map((v) => (
          <div key={v._id} className="card shadow p-2 relative">
            <div onClick={() => setPreviewPopup(v)} className="cursor-pointer">
              ...
              {v.type === "file" && (
                <video src={`https://localhost:5000/uploads/videos/${v.filename}`} className="w-full h-32" />
              )}
        ...
        <div className="fixed inset-0 bg-black bg-opacity-70 flex justify-center items-center z-50">
          <div className="bg-white p-4 max-w-3xl w-full relative">
            ...
            {previewPopup.type === "file" && (
              <video src={`https://localhost:5000/uploads/videos/${previewPopup.filename}`} controls autoPlay className="w-full" />
            )}
