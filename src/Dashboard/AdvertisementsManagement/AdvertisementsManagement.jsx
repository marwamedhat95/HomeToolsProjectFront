...
export default function AdvertisementsManagement() {
  ...
  const fetchAds = async () => {
    try {
      const res = await axios.get("https://localhost:5000/api/ads");
      setAds(res.data || []);
    } catch (err) {
      console.error(err);
    }
  ...
  const handleAddAd = async () => {
    ...
    setLoading(true);
    try {
      const res = await axios.post("https://localhost:5000/api/ads", formData, {
  ...
  const handleDelete = async (id) => {
    try {
      await axios.delete(`https://localhost:5000/api/ads/${id}`);
