...
buttonText: "",
buttonLink: "",
background: null, // الصورة الجديدة (رابط Cloudinary بعد الرفع)
currentBackground: "", // الصورة الموجودة (من الـ API)
});
...
// ملفات منفصلة لكل فورم عشان ما يتداخلش الرفع
...
background: res.data.background, // خلي background فيها رابط موجود لو حابة تعدلي
});
} catch (err) {
console.error(err);
}
...
// ---------------- VALIDATION ----------------
const validateForm = (form, filesArr) => {
if (
!form.name.trim() ||
!form.price ||
!form.quantity ||
...
// اختاري المصفوفة المناسبة حسب نوع الفورم
const filesToUpload = form.homeProduct ? homeFiles : offerFiles;


if (!validateForm(form, filesToUpload)) return;
...
try {
  ...
  const body = {
    ...form,
    images: uploadedUrls, // بدل الملفات نفسها، تبعث روابط Cloudinary
  };

  await axios.post("https://hometoolsprojectbackendd-production.up.railway.app/api/products", body);

  showPopup("تمت إضافة المنتج بنجاح 🎉", "success");

  // تفريغ الفورم و الملفات
  setForm({
    ...form,
    name: "",
    price: "",
  ...
  if (form.homeProduct) setHomeFiles([]);
  else setOfferFiles([]);
...
// ---------------- UPDATE PRODUCT (logic داخل زر الحفظ في المودال) ----------------
      ...
      <input
        ...
        onChange={async (e) => {
          ...
          try {
            ...
            showPopups("فشل رفع صورة الهيرو", "error");
          }
        }}
      />
    ...
    <div className="modal-overlay">
      <div className="modal-content">
        ...
        <button
          ...
          onClick={async () => {
            try {
              // لو اختارت صور جديدة - نرفعهم و نضع الروابط في الجسم
              if (editPopup.product.newImages && editPopup.product.newImages.length > 0) {
                const uploadedUrls = await Promise.all(
                  Array.from(editPopup.product.newImages).map(file => uploadImage(file))
                );
                // نرسل تحديث يحتوي على الصور الجديدة (الروابط)
                await axios.put(
                  `https://hometoolsprojectbackendd-production.up.railway.app/api/products/${editPopup.product._id}`,
                  { ...editPopup.product, images: uploadedUrls }
                );
              } else {
                // لو مفيش صور جديدة نرسل باقي الحقول كـ form-data أو json بسيط
                const body = {
              ...
              console.error(err);
              showPopup("حدث خطأ أثناء التعديل", "error");
            }
          }}
        >
          حفظ التعديل
